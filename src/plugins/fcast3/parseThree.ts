...
const parseThree = (body: string): Result => {
  const data: Data = {}
  let error_code
  ...
  if (!regex.test(body)) error_code = 3 // Update timestamp not found
    ...
    const dateTime = dayjs(`${match?.[1]} ${match?.[2]}`, 'DD.MM. HH:mm')
    ...
    if (regex.test(body)) {
      ...
      days.push(
        { date: dayjs(match?.[1], 'DD.MM.').format('YYYY-MM-DD') },
        { date: dayjs(match?.[2], 'DD.MM.').format('YYYY-MM-DD') },
        { date: dayjs(match?.[3], 'DD.MM.').format('YYYY-MM-DD') },
      )
